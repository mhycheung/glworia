...
def contour_int(T0, x_init, cond_func, step_func, y, lens_params):
    # dx_init = f(x_init, y, lens_params)
    u_acc = 0.
    # x_arr = jnp.array([x_init])
    init_val = [x_init, x_init, u_acc, u_acc, T0, 0, 0, y, lens_params]
    # init_val = [x_init, x_init, u_acc, x_arr]
...
# def make_contour_int_v(T, dT, dT_norm, f, T_hess_det, h):
#     cond_func = contour_int_cond_func_full
#     step_func = make_contour_int_step_func(T, dT, dT_norm, f, T_hess_det, h)
#     f_red = jit(lambda T0, x_init: contour_int(T0, T, dT, dT_norm, x_init, 
#                                                f, T_hess_det, h,
#                                                cond_func, step_func))
#     return vmap(f_red, (0, 0), 0)
...
class contour_init_1D:
    ...
    def __init__(self, crit_points, T0_arr, T_1D, T, dT, dT_norm, f,
        ...
        self.update_params(crit_points, T0_arr, y, lens_params, long_num, short_num, dense_width, cusp_points)
...
def find_outer(T0_arr, min_x0, T_sad, T_min, T_1D, args_1D, bisect_cond_fun, bisect_step_fun_T_1D, margin_fac = 1.5):
